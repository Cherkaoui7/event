...
const DECO_LABELS = {
  zellige: "Zellige traditionnel",
  traditionnel_marocain: "Marocain classique",
  moderne: "Moderne épuré",
};
const LIGHT_LABELS = {
  tamise: "Ambiance tamisée",
  spots: "Spots design",
  lustres_traditionnels: "Lustres traditionnels",
};
const LAYOUT_LABELS = {
  classique_rond: "Tables rondes",
  rectangulaire: "Tables rectangulaires",
  u_shape: "Disposition en U",
  cocktail: "Cocktail",
};
      ...
      <div style={s.container} className="print-container">
        <div style={s.header}>
          <div>
            ...
            >
              Devis Officiel - {new Date().toLocaleDateString("fr-FR")}
            </p>
          </div>
          <button
        ...
        <div style={s.card} className="print-card">
          ...
          <Row
            ...
            value={
              event.budget
                ? `${event.budget.toLocaleString()} MAD`
                : "Non défini"
            }
          />
          <Row
            label="Statut"
            value={event.status === "draft" ? "⏳ Brouillon" : "✅ Confirmé"}
          />
        </div>
            ...
            <div style={s.card} className="print-card">
              ...
              {room.decoration_style && (
                <Row
                  label="Décoration"
                  value={
                    DECO_LABELS[room.decoration_style] || room.decoration_style
                  }
                />
              )}
              {room.lighting_style && (
                <Row
                  label="Éclairage"
                  value={
                    LIGHT_LABELS[room.lighting_style] || room.lighting_style
                  }
                />
              )}
              {room.table_layout && (
                <Row
                  label="Disposition"
                  value={LAYOUT_LABELS[room.table_layout] || room.table_layout}
                />
              )}
            </div>
          )}
          ...
          <div style={s.card} className="print-card">
            ...
            {caterings.map((ec) => (
              <div key={ec.id} style={s.cateringRow}>
                ...
                <span style={{ fontWeight: "600", color: "#b76e4b" }}>
                  {ec.line_total?.toLocaleString()} MAD
                </span>
              </div>
            ))}
          </div>
        ...
        <div style={s.totalCard} className="print-card">
          ...
          <strong style={s.totalValue}>
            {event.total_price?.toLocaleString()} MAD
          </strong>
        </div>
