...
class PackSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if ($couscous) {
            $eco->packCaterings()->create(['catering_item_id' => $couscous->id,    'quantity_per_guest' => 1]);
        }
        if ($patisseries) {
            $eco->packCaterings()->create(['catering_item_id' => $patisseries->id, 'quantity_per_guest' => 1]);
        }
        ...
        if ($tajine) {
            $luxe->packCaterings()->create(['catering_item_id' => $tajine->id,  'quantity_per_guest' => 0.8]);
        }
        if ($pastilla) {
            $luxe->packCaterings()->create(['catering_item_id' => $pastilla->id, 'quantity_per_guest' => 0.5]);
        }
        if ($mechoui) {
            $luxe->packCaterings()->create(['catering_item_id' => $mechoui->id, 'quantity_per_guest' => 0.7]);
        }
        if ($the) {
            $luxe->packCaterings()->create(['catering_item_id' => $the->id,     'quantity_per_guest' => 1]);
        }
